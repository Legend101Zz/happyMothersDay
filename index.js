...
var layer1 =
  width - width / 2 / 50 - $(".layer1mountain").width() / 2 - width / 2;
var layer2 =
  width - width / 2 / 40 - $(".layer2mountain").width() / 2 - width / 2;
var layer3 =
  width - width / 2 / 30 - $(".layer3mountain").width() / 2 - width / 2;
var layer4 =
  width - width / 2 / 20 - $(".layer4mountain").width() / 2 - width / 2;
var layer5 =
  width - width / 2 / 15 - $(".layer4mountain").width() / 2 - width / 2;
...
function resetLayers() {
  ...
  $(".sunSVG").css({
    left: layer5,
    transform: "rotateY(" + sun + "deg)",
    "-webkit-transform": "rotateY(" + sun + "deg)",
  });
...
function nextSeason() {
  ...
  if (season == 1) {
    ...
    $(".mountains").animate({ opacity: 0.9 }, 1000);
    $(".sunSVG").animate({ height: height * 0.3, opacity: 1 }, 1000);
    ...
    setTimeout(function () {
      ...
      $(".seasonName").animate({ opacity: 0.3 }, 1000);
    }, 1000);
  }
  if (season == 2) {
    ...
    $(".sunSVG").animate(
      { height: height * 0.4 },
      {
        ...
        step: function (current, animProperties) {
          layer5 =
            width - lastMouseX / 15 - $(".sunSVG").width() / 2 - width / 2;
    ...
    $(".seasonName").animate({ opacity: 0 }, 1000);
    setTimeout(function () {
      $(".seasonName").text("Summer");
      $(".seasonName").animate({ opacity: 0.3 }, 1000);
    }, 1000);
  }
  if (season == 3) {
    $("body,html").animate({ backgroundColor: "#f0ae75" }, 500);
    $(".mountains").animate({ opacity: 0.7 }, 1000);
    $(".sunSVG").animate(
      { height: height * 0.3 },
      {
        duration: 500,
    ...
    $(".seasonName").animate({ opacity: 0 }, 1000);
    setTimeout(function () {
      $(".seasonName").text("Autumn");
      $(".seasonName").animate({ opacity: 0.3 }, 1000);
    }, 1000);
  }
  if (season == 4) {
    $("body,html").animate({ backgroundColor: "#121a21" }, 500);
    $(".snow").animate({ opacity: 0.9 }, 1000);
    $(".mountains").animate({ opacity: 0.3 }, 1000);
    $(".sunSVG").animate({ opacity: 0 }, 500);
    $(".seasonName").animate({ opacity: 0 }, 1000);
    setTimeout(function () {
      $(".seasonName").css({ color: "white" });
      $(".seasonName").text("Winter");
      $(".seasonName").animate({ opacity: 0.3 }, 1000);
    }, 1000);
  }
  if (season == 5) {
    $("body,html").animate({ backgroundColor: "#c1f1d5" }, 500);
    $(".snow").animate({ opacity: 0 }, 1000);
    $(".mountains").animate({ opacity: 0.2 }, 1000);
    $(".sunSVG").animate({ height: height * 0.3, opacity: 1 }, 1000);
    $(".seasonName").animate({ opacity: 0 }, 1000);
    setTimeout(function () {
      $(".seasonName").text("Spring");
    ...
    setTimeout(function () {
      $(".message").animate({ opacity: 0.5 }, 1000);
      setTimeout(function () {
        $(".happyMothersDay").animate({ opacity: 0.9 }, 1000);
        $(".seasonName").css({ opacity: 0 });
      }, 3000);
    }, 2000);
  }
